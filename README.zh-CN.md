...
对于正式验证的 `Offline-win-x64` 发布链路，若拆音首次运行时无法可靠写入应用目录，Demucs 运行时与模型也会回退到 `%LOCALAPPDATA%\Vidvix\Tools\Demucs` 下准备。
...
如果缺少单文件主程序所需的外置资产、`Tools` 依赖或本地化资源，发布过程会直接失败。

主要发布输出目录：

- 正式发布目录：`E:\SoftwareBuild\Vidvix\`
- Release 发布后同步生成的内部镜像目录：`artifacts/publish-offline/`
...
- 主离线发布配置使用 `PublishSingleFile=true`
- 主离线发布配置固定使用 `RuntimeIdentifier=win-x64`
- 主离线发布配置固定使用独立部署：`SelfContained=true`
- `Tools/` 下的外部运行时资产有意保留为独立文件，不进入 `Vidvix.exe` 单文件束包路径
- 正是因为采用单文件发布，拆音相关的 `Demucs` 运行时包才继续以外置包形式随包携带
