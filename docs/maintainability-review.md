...
- 为拆音模块新增 `SplitAudioWorkspacePreferencesState`、`SplitAudioProgressState`、`SplitAudioResultCollectionState`、`SplitAudioInputState` 与 `SplitAudioExecutionCoordinator`。
  `SplitAudioWorkspaceViewModel` 现在不再同时持有偏好解析、输出目录归一化、进度显示字段、结果集合可见性判断、输入路径 / 文件名 / 摘要 / 占位可见性，以及拆音工作流调用 / 结果文案翻译 / 自动定位输出；下一轮可以继续把预览生命周期再收口成独立门面。
