...
- 为拆音模块新增 `SplitAudioWorkspacePreferencesState`、`SplitAudioProgressState`、`SplitAudioResultCollectionState` 与 `SplitAudioInputState`。
  `SplitAudioWorkspaceViewModel` 现在不再同时持有偏好解析、输出目录归一化、进度显示字段、结果集合可见性判断，以及输入路径 / 文件名 / 摘要 / 占位可见性；这一轮先把最稳定的状态职责抽离出来，为下一轮继续拆执行协调器打基础。
