...
- 用户偏好字段：`Core/Models/UserPreferences.cs`（`CurrentUiLanguage` 现在可能是具体语言码，也可能是 `system`）
...
- 主要代码入口：
  ...
  - `ViewModels/AboutWorkspaceViewModel.cs`
- 适用场景：
  - 关于页顶部切换栏
  - 关于 / 许可证 / 隐私 三个分区标题
  - 分区占位说明
  - 后续补充的许可证与隐私注意内容
- 如果改的是关于模块在主窗口侧边栏或页头外壳中的入口文案，同时看：
...
2. 在 `Resources/Localization/zh-CN/*.json`、`Resources/Localization/en-US/*.json` 和 `Resources/Localization/zh-TW/*.json` 同时补 key。
...
- 关于：`AboutWorkspaceViewModel.RefreshLocalization()`
...
2. 复制现有 10 个 JSON 文件结构，保持 key 全量一致。
3. 更新 `manifest.json`，把新语言登记进去。
4. 在 `common.json` 中补语言显示名 key，例如 `common.language.option.ja-jp`；如果设置页提供“跟随系统”，要确认 `common.language.option.system` 在所有已支持语言里都存在。
5. 切换语言后跑一次往返验证，至少覆盖：
   - 设置页
   - 主窗口
   - AI 工作区
...
- `common.language.option.system`
- 含义：跟随系统
- 解析规则：系统为英语时落到 `en-US`，为简体中文时落到 `zh-CN`，为繁体中文时落到 `zh-TW`，其他系统语言统一回退 `zh-CN`
